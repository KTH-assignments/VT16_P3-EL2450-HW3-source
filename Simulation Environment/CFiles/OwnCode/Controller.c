// Calculate theta_R, the angle between the line that connects the robot and the
// goal, and the x-axis
theta_R = atan2( yg-y0, xg-x0 ) * 180 / PI;

// Choose control strategy
if ( fabs(theta - theta_R) > angle_tolerance )
{
  control_strategy = ROTATION_CONTROL;
}
else if ( fabs(cos(theta_R * PI / 180) * (xg - x)
    + sin(theta_R * PI / 180) * (yg - y)) > distance_tolerance )
{
  control_strategy = TRANSLATION_CONTROL;
}
else
{
  control_strategy = STOP_CONTROL;
}


/*control_strategy = ROTATION_CONTROL;*/
/*control_strategy = TRANSLATION_CONTROL;*/

switch ( control_strategy )
{
  case ROTATION_CONTROL:

    u_psi = r_K_psi * (theta_R - theta);

    // Task 8
    /*right = 0.5 * u_psi;*/
    /*left = -right;*/

    u_omega = r_K_omega * (cos(theta * PI / 180) * (x0 - x) + sin(theta * PI / 180) * (y0 - y));

    // Task 10
    /*left = u_omega;*/
    /*right = u_omega;*/

    break;

  case TRANSLATION_CONTROL:

    u_omega = t_K_omega * (cos(theta_R * PI / 180) * (xg-x)
      + sin(theta_R * PI / 180) * (yg-y));

    // Task 13
    /*left = u_omega;*/
    /*right = u_omega;*/

    u_psi = t_K_psi *
      (sin(theta_R * PI / 180) * (x + t_p * cos(theta * PI / 180) - x0)
       - cos(theta_R * PI / 180) * (y + t_p * sin(theta * PI / 180) - y0));

    // Task 16
    /*right = 0.5 * u_psi;*/
    /*left = -right;*/
    break;

  case STOP_CONTROL:

    u_psi = 0;
    u_omega = 0;
    break;
}

left = u_omega - 0.5 * u_psi;
right = u_omega + 0.5 * u_psi;
